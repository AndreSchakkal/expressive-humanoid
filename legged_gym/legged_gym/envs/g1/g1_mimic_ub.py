...
class G1MimicUB(LeggedRobot):
    ...
    def init_motions(self, cfg):
        ...
        self._key_body_ids_sim = torch.tensor([3, 4, 5, # Left Hip yaw, Knee, Ankle
                                                9, 10, 11,
                                                16, 19, 20, # Left Shoulder pitch, Elbow, hand
                                                21, 24, 25], device=self.device)     
...
# self._valid_dof_body_ids = torch.ones(len(self._dof_body_ids)+2*4, device=self.device, dtype=torch.bool)
        ...
        # self._valid_dof_body_ids[4] = 0    ## DISCARD ADDITIONAL ROTATION FOR left_ankle yaw  #MAYBE
        # self._valid_dof_body_ids[5] = 0    ## DISCARD ADDITIONAL ROTATION FOR right_ankle yaw        self._valid_dof_body_ids[6] = 0    ## DISCARD ADDITIONAL ROTATION FOR left_ankle yaw  #MAYBE
        # self._valid_dof_body_ids[11] = 0    ## DISCARD ADDITIONAL ROTATION FOR right_ankle yaw
        # self._valid_dof_body_ids[12] = 0    ## DISCARD ADDITIONAL ROTATION FOR right_ankle yaw
...
# self._valid_dof_body_ids[-6] = 0    ## I THINK THIS IS THE left_hand_keypoint_link
        ...
        # self.dof_indices_motion = torch.tensor([1, 0, 2,    5, 4, 6,   8,7,9,    16, 14, 15,     19, 17, 18,   24, 22, 23], device=self.device, dtype=torch.long)


        # self.dof_indices_sim = torch.tensor([0, 1, 2,    4, 5, 6,    7, 8, 9,  11,12,13,   14, 15, 16,    17, 18, 19,    22, 23, 24], device=self.device, dtype=torch.long)
        # self.dof_indices_motion = torch.tensor([1, 0, 2,    5, 4, 6,   8,7,9,    12,11,13,   15, 14, 16,     18, 17, 19,   23, 22, 24], device=self.device, dtype=torch.long)



        # self.dof_indices_motion = torch.tensor([1, 0, 2,    5, 4, 6,   8,7,9,    15, 14, 16,     18, 17, 19,   24, 23, 22], device=self.device, dtype=torch.long)
        # self.dof_indices_sim = torch.tensor([0, 1, 2, 4, 5, 6, 7, 8, 9, 14, 15, 16, 17, 18, 19, 22, 23, 24], device=self.device, dtype=torch.long)
...
#'right_shoulder_pitch_joint', 'right_shoulder_roll_joint', 'right_shoulder_yaw_joint', 'right_elbow_joint']
    ...
    def update_demo_obs(self):
        ...
        root_vel[:,[0,1,2]] = torch.tensor([0.,0.,0.]).to('cuda:0')
