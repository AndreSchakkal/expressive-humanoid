...
class G1MimicPriv(LeggedRobot):
    ...
    def compute_observations(self):
        ...
        # print(" self.root_states[:, 7:10].view(self.num_envs, -1)] ",  self.root_states[:, 7:10].view(self.num_envs, -1)])

        if self.cfg.terrain.measure_heights:
            heights = torch.clip(self.root_states[:, 2].unsqueeze(1) - 0.3 - self.measured_heights, -1, 1.)
            self.obs_buf = torch.cat([motion_features, obs_buf, obs_demo, heights, priv_explicit, priv_latent, self.obs_history_buf.view(self.num_envs, -1),  self.rigid_body_states[:, self._key_body_ids_sim[self._key_body_ids_sim_subset], :3].view(self.num_envs, -1), self.root_states[:, 7:10].view(self.num_envs, -1)], dim=-1)
        else:
            self.obs_buf = torch.cat([motion_features, obs_buf, obs_demo, priv_explicit, priv_latent, self.obs_history_buf.view(self.num_envs, -1),  self.rigid_body_states[:, self._key_body_ids_sim[self._key_body_ids_sim_subset], :3].view(self.num_envs, -1), self.root_states[:, 7:10].view(self.num_envs, -1)], dim=-1)
